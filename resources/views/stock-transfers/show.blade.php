...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-2">
        <div>
            <h4 class="fw-semibold mb-0">Stock Transfer <code>{{ $transfer->transfer_no }}</code></h4>
            ...
            @if($transfer->status == \App\Models\StockTransfer::STATUS_PENDING && $canActOnTransfer)
                @can('accept stock transfers')
                    <button class="btn btn-success stock-transfer-action"
                        ...
                        data-title="Accept Stock Transfer"
                        data-text="Stock will move from source to destination location.">
                        <i class="ti ti-check me-1"></i> Accept
                    </button>
                @endcan
                @can('reject stock transfers')
                    <button class="btn btn-label-danger stock-transfer-action"
                        data-url="{{ route('admin.stock-transfers.reject', $transfer) }}"
                        data-title="Reject Stock Transfer"
