...
<form id="commonModalForm" action="{{ route('admin.roles.update', $role) }}" method="POST" class="d-flex flex-column flex-grow-1">
    @csrf
    @method('PUT')
    <div class="mb-4">
    ...
    <div class="mb-4">
        ...
        <select id="roleLocation" name="location_id" class="form-select select2" required>
            ...
            @foreach($locations as $loc)
                <option value="{{ $loc->id }}" {{ $role->location_id == $loc->id ? 'selected' : '' }}>
                    {{ $loc->name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="d-flex gap-2 mt-auto pt-3 border-top">
        <button type="submit" class="btn btn-primary w-50">Update Role</button>
