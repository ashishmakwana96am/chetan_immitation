    ...
    <script>
        $(document).ready(function () {
            ...
            const table = $('#bankLedgerTable').DataTable({
                    ...
                    dataSrc: function (json) {
                        ...
                        if (json.current_balance !== undefined) {
                            ...
                            $('#current-balance-container').html(
                                '<span class="badge ' + (isNeg ? 'bg-label-danger' : 'bg-label-primary') + ' fs-6 fw-bold" style="display: flex; justify-content: center; align-items: center;"><i class="ti ti-building-bank me-1"></i> Bank Balance: ' + json.current_balance + '</span>'
                            );
                        }
                        return json.data;
                    },
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            if (type === 'sort' || type === 'type') {
                                return row.raw_receipt !== undefined ? parseFloat(row.raw_receipt) : (parseFloat(String(data).replace(/[^0-9.-]+/g, '')) || 0);
                            }
                            return data;
                        }
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            if (type === 'sort' || type === 'type') {
                                return row.raw_payment !== undefined ? parseFloat(row.raw_payment) : (parseFloat(String(data).replace(/[^0-9.-]+/g, '')) || 0);
                            }
                            return data;
                        }
                    },
            ...
            $(document).on('click', '#exportExcelBtn', function () {
                ...
                fetch("{{ route('admin.ledgers.bank.export') }}?" + params.toString(), {
                    method: 'GET',
                    headers: {
                ...
                .then(response => {
                    ...
                    let filename = 'bank_ledger_' + new Date().toISOString().slice(0,10) + '.xlsx';
