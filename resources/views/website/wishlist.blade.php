    ...
    <div class="container-1440">
                        ...
                        <div class="flex-1 min-w-0 flex flex-col justify-between">
                            ...
                            <div class="border-t border-[#D5D5D5] mt-2 pt-2">
                                <div class="flex items-center flex-wrap gap-2 text-[#3D403F]">
                                    @if($stockQty < 1)
                                    <button class="add-to-cart-from-wishlist after:border-r after:border-[#3D403F] after:h-5 after:sm:mx-4 after:content-[''] after:inline-block flex items-center gap-[10px] text-sm sm:text-base opacity-40 cursor-not-allowed pointer-events-none"
                                        data-product-id="{{ $prod->id }}"
                                        data-variant-id="{{ $variant?->id ?? '' }}"
                                        disabled>
        ...
        <div class="mt-20 border-t border-[#D5D5D5] pt-16">
            ...
            <div class="product-list-toggle-wrapper" data-product-view-toggle>
                ...
                <div data-product-grid class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
...
$(function () {
    ...
    $(document).on('click', '.add-to-cart-from-wishlist', function () {
        var btn       = this;
        if ($(btn).prop('disabled')) return; // sold out — do nothing
    ...
    $(document).on('click', '#moveAllToCartBtn', function () {
        ...
        $('#wishlistItems .wishlist-item').each(function () {
            ...
            items.push({
                ...
                disabled: addBtn.prop('disabled'),
            });
        });

        if (!items.length) return;

        // Filter out sold out items (disabled buttons)
        items = items.filter(function (item) {
            return !item.disabled;
        });

        if (!items.length) {
            if (window.showWishlistToast) window.showWishlistToast('All items are sold out.', false);
            return;
        }
                ...
                '<div class="flex flex-col sm:flex-row gap-4 group">' +
                    ...
                    '</div>' +
                        '<div class="border-t border-[#D5D5D5] mt-2 pt-2">' +
                            '<div class="flex items-center flex-wrap gap-2 text-[#3D403F]">' +
                                (p.out_of_stock
                                    ? '<button class="add-to-cart-from-wishlist after:border-r after:border-[#3D403F] after:h-5 after:sm:mx-4 after:content-[\'\'] after:inline-block flex items-center gap-[10px] text-sm sm:text-base opacity-40 cursor-not-allowed pointer-events-none" data-product-id="' + p.product_id + '" data-variant-id="' + (p.variant_id || '') + '" disabled><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4 sm:size-5 -mt-0.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" /></svg>Sold Out</button>'
                                    : '<button class="add-to-cart-from-wishlist after:border-r after:border-[#3D403F] after:h-5 after:sm:mx-4 after:content-[\'\'] after:inline-block hover:text-[#B4771E] flex items-center gap-[10px] text-sm sm:text-base transition" data-product-id="' + p.product_id + '" data-variant-id="' + (p.variant_id || '') + '"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4 sm:size-5 -mt-0.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" /></svg>Add To Cart</button>'
                                ) +
                                '<button class="remove-wishlist-btn hover:text-red-500 flex items-center gap-[10px] text-sm sm:text-base transition"' +
                                    ' data-product-id="' + p.product_id + '" data-variant-id="' + (p.variant_id || '') + '" data-toggle-url="' + p.toggle_url + '">' +
                                    '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4 sm:size-4 sm:size-5 -mt-0.5"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>Remove From Wishlist' +
                                '</button>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
